...
mod tests {
    use rust_git::vcs::{
        files::repository::Repository, version_control_system::VersionControlSystem,
    };
    use std::{fs, io::Write};
    ...
    pub fn test_01_untracked_files_contains_four_files() {
        ...
        assert!(
            matches!(VersionControlSystem::status(), Ok((untracked_files, _, _)) if untracked_files.len() == 4)
        );
    }
    ...
    pub fn test_02_untracked_files_contains_one_file_with_status_created() {
        ...
        assert!(
            matches!(VersionControlSystem::status(), Ok((untracked_files, _, _)) if status_contains(untracked_files.clone(), "CREATED", &path))
        );
    }
    ...
    pub fn test_04_changes_not_staged_for_commit_contains_one_file() -> Result<(), std::io::Error> {
        ...
        assert!(
            matches!(VersionControlSystem::status(), Ok((_, changes_not_staged_for_commit, _)) if status_contains(changes_not_staged_for_commit.clone(), "MODIFIED", &path))
        );
        assert!(
            matches!(VersionControlSystem::status(), Ok((_, changes_not_staged_for_commit, _)) if changes_not_staged_for_commit.len() == 1)
        );
        Ok(())
    }
    ...
    pub fn test_05_changes_not_staged_for_commit_contains_one_file() -> Result<(), std::io::Error> {
        ...
        assert!(
            matches!(VersionControlSystem::status(), Ok((_, changes_not_staged_for_commit, _)) if status_contains(changes_not_staged_for_commit.clone(), "DELETED", &file.to_path_buf()))
        );
        assert!(
            matches!(VersionControlSystem::status(), Ok((_, changes_not_staged_for_commit, _)) if changes_not_staged_for_commit.len() == 1)
        );
    ...
    pub fn test_06_untracked_file_contain_file_1_and_changes_not_staged_for_commit_contain_file_3(
        ...
        assert!(
            matches!(VersionControlSystem::status(), Ok((_, changes_not_staged_for_commit, _)) if status_contains(changes_not_staged_for_commit.clone(), "DELETED", &file_3.to_path_buf()))
        );
        assert!(
            matches!(VersionControlSystem::status(), Ok((untracked_file, _, _)) if status_contains(untracked_file.clone(), "CREATED", &file_1))
        );
        Ok(())
    }
    ...
    pub fn test_07_untracked_file_contain_file_1_and_changes_not_staged_for_commit_contain_file_2_and_file_3(
        ...
        assert!(
            matches!(VersionControlSystem::status(), Ok((_, changes_not_staged_for_commit, _)) if status_contains(changes_not_staged_for_commit.clone(), "MODIFIED", &file_2))
        );
        assert!(
            matches!(VersionControlSystem::status(), Ok((_, changes_not_staged_for_commit, _)) if status_contains(changes_not_staged_for_commit.clone(), "DELETED", &file_3.to_path_buf()))
        );
        ...
        assert!(
            matches!(VersionControlSystem::status(), Ok((untracked_file, _, _)) if untracked_file.len() == 1)
        );
        assert!(
            matches!(VersionControlSystem::status(), Ok((_, changes_not_staged_for_commit, _)) if changes_not_staged_for_commit.len() == 2)
        );
        Ok(())
    }
    ...
    pub fn test_08_staging_area_contain_file_1() {
        ...
        assert!(
            matches!(VersionControlSystem::status(), Ok((_, _, changes_to_be_commited)) if status_contains(changes_to_be_commited.clone(), "CREATED", &path))
        );
        assert!(
            matches!(VersionControlSystem::status(), Ok((untracked_file, _, _)) if untracked_file.is_empty())
        );
    }
    ...
    pub fn test_09_staging_area_contain_file_1_and_changes_not_staged_for_commit_contain_file1(
        ...
        let (untracked_files, changes_not_staged_for_commit, changes_to_be_commited) =
            VersionControlSystem::status()?;

        assert_eq!(
            status_contains(changes_to_be_commited.clone(), "MODIFIED", &path),
            true
        );
        assert_eq!(untracked_files.len(), 0);
    ...
    pub fn test_12_three_sets_are_empty() -> Result<(), std::io::Error> {
        ...
        VersionControlSystem::add(&path)?;
        let (untracked_file, changes_not_staged_for_commit, changes_to_be_commited) =
            VersionControlSystem::status()?;
