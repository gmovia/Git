use crate::{
    constants::constant::{
        BLOB_CODE, BOTH, CURRENT, INCOMING, RESPONSE_OK_CLONE, RESPONSE_OK_REMOTE,
    },
    handlers::clone::handler_clone,
    vcs::{
        commands::{
            hash_object::WriteOption,
            remote::{Remote, RemoteOption},
            show_ref::ShowRefOptions,
            tag::TagOptions,
        },
        entities::{
            change::{read_changes, write_changes, Change},
            conflict::Conflict,
        },
        files::{current_repository::CurrentRepository, log::Log},
...
use super::{
    draw::{changes_and_staging_area, draw_error, draw_message, draw_push_pull_fetch},
    handler_button::{
        handle_button_select_branch, handle_button_select_repository, handle_buttons_branch,
        handle_buttons_repository, handle_check_ignore_button, handle_commit_button,
        handle_ls_files_buttons, handle_ls_tree_button, handle_rm_button, handle_terminal,
    },
...
pub fn handle_repository(interface: &RustInterface) {
    ...
    interface.repository_button.connect_clicked(move |_| {
...
pub fn handle_branch(interface: &RustInterface) {
    ...
    interface.branch_button.connect_clicked(move |_| {
        rc_box.foreach(|child| {
            rc_box.remove(child);
        });
        dialog.run();
    ...
    interface.merge.connect_clicked({
        ...
        move |button| {
            ...
            if let Ok(conflicts) = VersionControlSystem::merge(&m_entry.text()) {
                    ...
                    button_resolve.connect_clicked({
                        ...
                        move |button| {
                            ...
                            for (key, value) in &conflicts {
                                if let (Ok(cat_current), Ok(cat_incoming)) = (
                                    VersionControlSystem::cat_file(&value.change_current.hash),
                                    VersionControlSystem::cat_file(&value.change_branch.hash),
                                ) {
                                    let set_label_current =
                                        format!("{}\n             {}\n", key, cat_current);
                                    let set_label_incoming =
                                        format!("\n{}\n            {}\n", key, cat_incoming);
                    ...
                    apply_c.connect_clicked({
                        ...
                        move |button| {
                            ...
                            if let Ok(list_conflicts) = read_changes() {
                                let _ = VersionControlSystem::resolve_conflicts(
                                    &m_entry1.text(),
                                    list_conflicts,
                                );
                            }
                ...
                draw_error(
                    rc_tuple.clone(),
                    format!("ERROR:  CANNOT FOUND THE BRANCH {}", m_entry.text()),
                    &m_entry,
                );
            }
            m_entry.set_text("");
...
pub fn add_both(
    ...
    dialog: &gtk::Dialog,
    both_box: &gtk::Box,
    both_text: &gtk::TextView,
    both_ok: &gtk::Button,
    buttons: (gtk::Button, gtk::Button, gtk::Button),
) {
    let conflict_c = conflict.clone();
            ...
            both_ok.connect_clicked({
                ...
                move |_| {
                    if let Some(result) = both_text.buffer() {
                        if let Some(result) =
                            result.text(&result.start_iter(), &result.end_iter(), false)
                        {
                            if let Ok(current) = CurrentRepository::read() {
                                    ...
                                    .open(&temp_path)
                                {
                                    ...
                                    if let Ok(hash) = VersionControlSystem::hash_object(
                                        &temp_path,
                                        WriteOption::Write,
                                        BLOB_CODE,
                                    ) {
                                        ...
                                        let change_current = Change {
                                            file: conflict_c.file.to_string(),
                                            hash: hash.clone(),
                                            state: conflict_c.change_current.state.clone(),
                                        };
                                        let change_branch = Change {
                                            file: conflict_c.file.to_string(),
                                            hash: hash.clone(),
                                            state: conflict_c.change_branch.state.clone(),
                                        };
                                        let conflict = Conflict {
                                            file: conflict_c.file.clone(),
                                            change_current,
                                            change_branch,
                                            resolved: BOTH.to_string(),
                                        };
                                        let _ = write_changes(&conflict);
                                    }
                                }
    ...
    interface.delete_tag_button.connect_clicked({
        ...
        move |button| {
            ...
            if VersionControlSystem::tag(TagOptions::Delete(&t_entry.text())).is_ok() {
                ...
                draw_error(
                    rc_tuple.clone(),
                    format!("ERROR:  CANNOT FOUND THE TAG {}", t_entry.text()),
                    &t_entry,
                );
            }
            tl_box.set_visible(true);
...
pub fn handle_remote(interface: &RustInterface) {
    ...
    interface.enter_add_remote.connect_clicked({
        ...
        move |_| {
            ...
            if let Ok(response) = VersionControlSystem::remote(RemoteOption::Add(
                n_add_entry.text().as_str(),
                p_entry.text().as_str(),
            )) {
                if response == RESPONSE_OK_REMOTE {
                    draw_message(
                        &a_box,
                        &"     ADD REMOTE SUCCESSFULLY!    ".to_string(),
                        0.5,
                    );
                } else {
                    draw_error(
                        rc_tuple.clone(),
                        "ERROR:  CAN'T REMOTE".to_string(),
                        &n_add_entry,
                    );
                }
                p_entry.set_text("");
    ...
    interface.delete_repo_remote.connect_clicked({
        ...
        move |_| {
            ...
            if let Ok(response) =
                VersionControlSystem::remote(RemoteOption::Remove(n_remove_entry.text().as_str()))
            {
                if response == RESPONSE_OK_REMOTE {
                    draw_message(
                        &r_box,
                        &"     REMOVE REMOTE SUCCESSFULLY!    ".to_string(),
                        0.5,
                    );
                } else {
                    draw_error(
                        rc_tuple.clone(),
                        "ERROR:  CAN'T REMOVE IN REMOTE".to_string(),
                        &n_remove_entry,
                    );
                }
    ...
    interface.get_repo_remote.connect_clicked({
        ...
        move |_| {
            ...
            if let Ok(path) =
                VersionControlSystem::remote(RemoteOption::Get(n_get_entry.text().as_str()))
            {
                draw_message(&g_box, &path.to_string(), 0.5);
            } else {
                draw_error(
                    rc_tuple.clone(),
                    "ERROR:  CAN'T GET THE REMOTES PATH".to_string(),
                    &n_get_entry,
                );
            }
...
pub fn handle_rebase(interface: &RustInterface) {
    ...
    interface.rebase_enter.connect_clicked({
        move |button| {
            ...
            if VersionControlSystem::rebase(&r_entry.text()).is_ok() {
                draw_message(&r_box, &"     REBASE SUCCESSFULLY!    ".to_string(), 0.5);
            } else {
                draw_error(
                    rc_tuple.clone(),
                    format!("ERROR:  CANNOT FOUND THE BRANCH NAME {}", r_entry.text()),
                    &r_entry,
                );
            }
            button.set_sensitive(false);
...
pub fn handle_clone(interface: &RustInterface) {
    ...
    interface.clone.connect_clicked({
        ...
        move |button| {
            ...
            if handler_clone(format!("git clone {}", &c_entry.text().to_string()))
                == RESPONSE_OK_CLONE
            {
                ...
                draw_error(
                    rc_tuple.clone(),
                    format!("ERROR:  CAN'T CLONE THE REPOSITORY {}", c_entry.text()).to_string(),
                    &c_entry,
                );
            }
            c_entry.set_text("");
