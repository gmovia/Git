...
impl CommitEntity{
    ...
    pub fn read(repo_path: &Path, commit_hash: &str) -> Result<CommitEntity, std::io::Error>{
        ...
        if second_line[0] != "parent" {
            return Ok(CommitEntity{content_type: tree_info[0].to_string(), tree_hash: tree_info[1].to_string(), parent_hash: COMMIT_INIT_HASH.to_owned(), author: commit_lines[2].to_string(), committer: commit_lines[3].to_string(),  message: commit_lines[4].to_string() })
        }
