...
use crate::{
    constants::constant::RESPONSE_OK_REMOTE, vcs::files::current_repository::CurrentRepository,
};
...
impl Remote {
    ...
    pub fn remote(current_repo: &Path, option: RemoteOption) -> Result<String, std::io::Error> {
        match option {
            RemoteOption::Add(repo_name_to_process, server_repo) => Ok(Remote::write_config(
                current_repo,
                repo_name_to_process,
                Path::new(server_repo),
            )?),
            RemoteOption::Remove(repo_name_to_process) => {
                Ok(Remote::remote_remove(current_repo, repo_name_to_process)?)
            }
            RemoteOption::Get(repo_name_to_process) => Ok(Remote::get_path_of_repo_remote(
                current_repo,
                repo_name_to_process,
            )?),
        }
    ...
    fn write_config(
        ...
        let mut config_file = OpenOptions::new()
            ...
            .create(true)
            .append(true)
            .open(Init::get_current_config(current_repo)?)?;
    ...
    pub fn get_path_of_repo_remote(
        current_repo: &Path,
        repo_name: &str,
    ) -> Result<String, std::io::Error> {
        let path = Init::get_current_config(current_repo)?;
        ...
        for (index, line) in content_split.iter().enumerate() {
            if line.contains(repo_name) {
                let parts: Vec<&str> = content_split[index + 1]
                    .split('=')
                    .map(|s| s.trim())
                    .collect();
