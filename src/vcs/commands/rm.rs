use crate::{
    constants::constant::{NULL, STATE_DELETED},
    utils::files::file::read,
    vcs::files::{index::Index, repository::Repository, vcs_file::VCSFile},
};
    ...
    pub fn rm_(path: &Path) -> Result<HashMap<String, VCSFile>, std::io::Error> {
        if path.is_dir() {
            return Err(std::io::Error::new(
                ...
                format!("fatal: not removing '{:?}' recursively without -r", path),
            ));
        }
        ...
        if let Ok(files) = read(path) {
            if files.is_empty() {
                return Err(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    format!("fatal: pathspec '{:?}' did not match any files", path),
                ));
            }
            for key in files.keys() {
                if Repository::read_repository()?.contains_key(key) {
                    ...
                    let file =
                        VCSFile::new(key.clone(), NULL.to_string(), STATE_DELETED.to_string());
    ...
    pub fn rm_r(dir_path: &Path) -> Result<HashMap<String, VCSFile>, std::io::Error> {
        ...
        if let Ok(files) = read(dir_path) {
            if files.is_empty() {
                return Err(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    format!("fatal: pathspec '{:?}' did not match any files", dir_path),
                ));
            }
            for key in files.keys() {
