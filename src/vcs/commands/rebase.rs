...
use crate::vcs::{
    ...
    files::{
        ...
        current_repository::CurrentRepository, repository::Repository, config::Config,
    },
};
use std::{
    collections::HashMap,
...
impl Rebase {
    ...
    pub fn rebase(branch: &str) -> Result<(), std::io::Error> {
        ...
        CurrentCommit::write_for_branch(&current, &current_branch, CurrentCommit::read_for_branch(&current, branch)?)?;
        ...
        for commit in commits_rebase {
            ...
            CommitsTable::write(&current, &current_branch, &commit.message, config.clone(), &repository_last_commit)?;
        }
