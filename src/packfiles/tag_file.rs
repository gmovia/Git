...
use crate::{
    packfiles::packfile::to_pkt_line,
    vcs::{
...
pub fn process_tag_directory(
    ...
    for entrada in fs::read_dir(path)? {
        ...
        if entry_path.is_file() {
            let data: (String, usize, usize) =
                process_tag_file(&entry_path, path_to_read, tag_to_send.clone())?;
            if data.1 != 0 {
