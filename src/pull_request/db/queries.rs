...
use crate::{
    pull_request::{
        schemas::schema::{CommitsPullRequest, PullRequestEntry},
        utils::path::{create_prs_file, create_table},
    },
    ...
    vcs::{
        commands::{merge::Merge, rebase::Rebase},
        entities::{commit_entity::CommitEntity, commit_table_entry::CommitTableEntry},
        files::commits_table::CommitsTable,
    },
};
...
impl Query {
    ...
    pub fn find_all_pull_requests(
        prs_path: &Path,
    ) -> Result<Vec<PullRequestEntry>, std::io::Error> {
    ...
    pub fn get_commits_pull_request(
        server: &Path,
        id: &Path,
    ) -> Result<Vec<CommitsPullRequest>, std::io::Error> {
    ...
    pub fn merge_pull_request(
        server: &Path,
        id: &Path,
        merge_method: &mut Option<String>,
    ) -> Result<String, std::io::Error> {
        ...
        if let Some(method) = merge_method {
            ...
            match method.as_str() {
                ...
                _ => Err(io::Error::new(
                    io::ErrorKind::Other,
                    "422 The method is wrong for merge a pull request",
                )),
            }
        } else {
            Err(io::Error::new(
    ...
    pub fn update_pr(
        base_commits_table: Vec<CommitTableEntry>,
        pr_entry: &mut PullRequestEntry,
        id: &Path,
    ) -> Result<String, std::io::Error> {
