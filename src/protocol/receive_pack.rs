...
use crate::packfiles::tag_file::{create_tag_files, exclude_tag_ref, get_tags};
...
use crate::utils::files::file::{create_file_and_their_folders, delete_all_files_and_folders};
...
fn extract_last_commit(log_content: &str) -> Result<String, io::Error> {
    ...
    let last_line = log_content.lines().last().ok_or(io::Error::new(
        io::ErrorKind::InvalidData,
        "Log file is empty",
    ))?;
    let line_parts: Vec<&str> = last_line.split('-').collect();
    let last_commit = line_parts.get(2).ok_or(io::Error::new(
        io::ErrorKind::InvalidData,
...
fn empty_log_file(server_client_path: &Path, branch_name: &str) -> io::Result<()> {
    let logs_path = server_client_path
        .join(".rust_git")
        .join("logs")
        .join(branch_name);
...
fn change_current_branch(
    receive_refs: Vec<String>,
    server_client_path: &Path,
) -> Result<(), std::io::Error> {
...
pub fn create_new_branch_with_hash(
    path: &Path,
    branch_name: &str,
    hash: &str,
) -> Result<(), std::io::Error> {
    let branch_head_path = path
        .join(".rust_git")
        .join("refs")
        .join("heads")
        .join(branch_name);
    let mut branch_head = OpenOptions::new()
        .write(true)
        .create(true)
        .append(false)
        .open(branch_head_path)?;
...
pub fn updating_repo(
    objects: Vec<(u8, Vec<u8>)>,
    repo_server_client: &Path,
    _last_commit: String,
) -> Result<(), std::io::Error> {
    ...
    for (commit_hash, commit_entity) in &hashes_sorted {
        write_commit_log_push(
            &commit_entity.parent_hash,
            commit_hash,
            commit_entity,
            repo_server_client.to_path_buf(),
        )?;
    }
...
fn write_commit_log_push(
    last_commit_hash: &String,
    new_commit_hash: &String,
    commit_entity: &CommitEntity,
    repo_server_client: PathBuf,
) -> Result<(), std::io::Error> {
    let id = Random::random();
    let mut commits_file = OpenOptions::new()
        .write(true)
        .append(true)
        .open(Init::get_current_log(&repo_server_client)?)?; //abro la tabla de commits para escribir - si no existe, la creo
...
pub fn read(repo_path: &Path) -> Result<HashMap<String, String>, std::io::Error> {
    ...
    local_repository.extend(read_repository_of_commit(
